...
export default class CardComponent extends Component {
  ...
  render() {
    const { item, linkClickHandler } = this.props;
    ...
    if (subList && Array.isArray(subList)) {
      const subListHtml = subList.map((subItem) => {
        const clickHandler = linkClickHandler ? linkClickHandler.bind(this, id, subItem.id) : null;
    ...
    const itemClickHandler = linkClickHandler ? linkClickHandler.bind(this, id) : null;
    return (
